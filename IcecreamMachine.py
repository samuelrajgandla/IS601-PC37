...
# add an empty _init_.py to the tests folder
from IcecreamExceptions import (
...
class Usable:
    ...
    def use(self):
        ...
        try:
            ...
            print(
                "The option that you chose is out of stock. Pick something else from the options"
            )
...
class IceCreamMachine:
    ...
    containers = [
        Container(name="Waffle Cone", cost=1.5),
        Container(name="Sugar Cone", cost=1),
        Container("Cup", cost=1),
    ]
    flavors = [
        Flavor(name="Vanilla", quantity=100, cost=1),
        Flavor(name="Chocolate", quantity=100, cost=1),
        Flavor(name="Strawberry", quantity=100, cost=1),
    ]
    toppings = [
        Toppings(name="Sprinkles", quantity=200, cost=0.25),
        Toppings(name="Chocolate Chips", quantity=200, cost=0.25),
        Toppings(name="M&Ms", quantity=200, cost=0.25),
        Toppings(name="Gummy Bears", quantity=200, cost=0.25),
        Toppings(name="Peanuts", quantity=200, cost=0.25),
    ]
    ...
    def pick_container(self, choice):
        ...
        try:
            ...
            print(
                f'"{choice}" is not a valid option, Select an option only from the list'
            )
    ...
    def pick_flavor(self, choice):
        try:
            ...
            print(
                f"Limit of the Ice Cream Machine is exceeded({self.USES_UNTIL_CLEANING}). Machine needs to be cleaned before making any extra icecreams."
            )
            # confirm to clean and continue or else quit
            clean = input("Would you like to (continue or quit)\n")
            if clean == "continue":
                self.clean_machine()
            ...
            print(
                f"The limit of maximum scoops that you can chose ({self.MAX_SCOOPS}) has done. Please continue with the toppings!"
            )
        ...
        try:
            ...
            print(
                f'"{choice}" it is not a valid Choice, Pick from the available choices in the list'
            )
    ...
    def pick_toppings(self, choice):
        try:
            ...
            print(
                f"The limit of maximum toppings that you can choose({self.MAX_TOPPINGS}) has done. Continue with the payment."
            )
    ...
    def handle_pay(self, expected, total):
        if total == str(expected):
            print("Thanks for choosing us! Yummy icecream is done!")
            ...
            try:
                ...
                print("The amount that you entered is wrong. Please enter the correct amount")
                self.run()
    ...
    def run(self):
        if self.currently_selecting == STAGE.Container:
            container = input(
                f"What would you like to take {', '.join(list(map(lambda c:c.name.lower(), filter(lambda c: c.in_stock(), self.containers))))}?\n"
            )
            self.handle_container(container)
        elif self.currently_selecting == STAGE.Flavor:
            flavor = input(
                f"What would you like to take {', '.join(list(map(lambda f:f.name.lower(), filter(lambda f: f.in_stock(), self.flavors))))}? Or type next.\n"
            )
            self.handle_flavor(flavor)
        elif self.currently_selecting == STAGE.Toppings:
            toppings = input(
                f"What would you like to take {', '.join(list(map(lambda t:t.name.lower(), filter(lambda t: t.in_stock(), self.toppings))))}? Or type done.\n"
            )
            self.handle_toppings(toppings)
        elif self.currently_selecting == STAGE.Pay:
            expected = self.calculate_cost()
            total = input(f"The total value is {expected}, please enter the same value.\n")
            self.handle_pay(expected, total)
            choice = input("Would you like to do more? (icecream or quit)\n")
            if choice == "quit":
                exit()
        self.run()
